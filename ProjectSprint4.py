...
if uploaded_file or len(st.session_state.messages) > 0:
    ...
    with st.form("chat_input_form", clear_on_submit=True, border=False):
        ...
        with col1:
            text_prompt = st.text_input("Type your request:", label_visibility="collapsed", placeholder="Ask me to translate this...")
    ...
    if submit_btn:
        ...
        if audio_value is not None:
            with chat_container: # Put the spinner in the chat area
        ...
        if prompt:
            ...
            # 2. Render new messages dynamically inside the container (prevents need for st.rerun)
            with chat_container:
